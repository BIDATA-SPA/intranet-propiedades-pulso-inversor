...
const ContactFormDialog = ({
  ...
  const onSubmit = async (
    ...
    try {
      await sendExchangeEmailRequest(data).unwrap()
      setSubmitting(false)
      onClose()
    } catch (err) {
  ...
  return (
    <Formik
      initialValues={{
        ...
        to: `${realtorInfo?.session?.email}`,
        propertyId: '',
        phone: '',
        realtorFrom: `${user?.session?.email}`,
        subject: 'Solicitud de Contacto - Buzón de Propiedades',
        message: `Estimado/a ${realtorInfo?.name} ${realtorInfo.lastName}, 
            ...
            <div className="grid grid-cols-1 gap-5">
              ...
              >
                <Field
                  ...
                  component={Input}
                  readOnly={false} // set field as false
                />
              </FormItem>
            </div>
