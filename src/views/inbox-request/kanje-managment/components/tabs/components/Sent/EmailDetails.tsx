...
const TimeLineComponent = ({ email, realtorOwner, requestingRealtor }) => {
...
const MainContent = ({ currentEmail }) => {
  const sentEmails =
    currentEmail && currentEmail?.emailsSent.length > 0 ? (
      currentEmail.emailsSent.map((email, index) => (
        <TimeLineComponent
          key={index}
          email={email}
          realtorOwner={currentEmail?.realtorOwner}
          requestingRealtor={currentEmail?.requestingRealtor}
        />
      ))
    ) : (
