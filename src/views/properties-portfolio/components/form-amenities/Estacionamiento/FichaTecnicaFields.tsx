  ...
  return (
    <>
      <div className="w-full grid grid-cols-1 gap-0 md:grid-cols-2 xl:grid-cols-3 md:gap-3">
        ...
        <FormItem label="Acceso a estacionamiento">
          <Field name="characteristics.accessToParking">
            {({ field, form }: FieldProps) => {
              return (
                <Select
                  isClearable
                  field={field}
                  options={filterAccessToParking}
                  placeholder="Seleccionar..."
                  value={filterAccessToParking?.filter(
                    (option) =>
                      option.value === values.characteristics?.accessToParking
                  )}
                  onChange={(option) => {
        ...
        <FormItem label="Tipo de Covertura">
