...
const EXTRA_STEPS: Step[] = [{ label: 'Subir Imágenes', value: 4 }]
...
const FormStep = ({
  ...
  const steps = useMemo(() => {
    return hasPropertyId ? [...BASE_STEPS, ...EXTRA_STEPS] : BASE_STEPS
  }, [hasPropertyId])

  const onStepChange = (step: number) => {
    const selectedStatus = stepStatus?.[step]?.status

    // permitir navegar libremente a pasos completados o current
    if (selectedStatus === 'complete' || selectedStatus === 'current') {
      dispatch(setCurrentStep(step))
      return
    }

    // permitir volver hacia atrás (sin saltar a futuros "locked")
    if (step < currentStep) {
      // si el paso actual estaba pending pero el usuario vuelve, puedes marcar current como complete (opcional)
      if (currentStepStatus === 'pending') {
        dispatch(setStepStatus({ [currentStep]: { status: 'complete' } }))
      }
      dispatch(setCurrentStep(step))
    }
  }
  ...
  return (
    <Menu variant="transparent" className="px-2">
      {steps.map((step: Step) => {
        const status = stepStatus?.[step.value]?.status

        return (
          <Menu.MenuItem
            key={step.value}
            eventKey={String(step.value)}
            className="mb-2"
            isActive={currentStep === step.value}
            onSelect={() => onStepChange(step.value)}
          >
            <span className="text-2xl ltr:mr-2 rtl:ml-2">
              {status === 'complete' && <HiCheckCircle className={textTheme} />}
              {(status === 'current' ||
                (status === 'pending' && currentStep === step.value)) && (
