...
const BrandsTable: React.FC = () => {
  ...
  const data = useAppSelector((state) => state.brandsList)
  const { data: brands, isLoading } = useGetServiceRequestBrandQuery({
    page: data.page,
    limit: data.limit,
  })
  const [
  ...
  useEffect(() => {
    if (brands) {
      ...
      dispatch(setTotalItems(brands?.meta?.totalItems))
    }
    dispatch(setLoading(isLoading))
  }, [brands, isLoading, dispatch])
