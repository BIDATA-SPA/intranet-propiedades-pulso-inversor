      ...
      <div className="w-full flex m-3 ml-4 text-white">
        ...
        <div className="title mt-11 ml-3 font-bold flex flex-col z-20">
          <div className="break-words text-white">
            <p>
              {realtor?.session?.rol && realtor?.session?.rol?.name}{' '}
            ...
            <div className="mt-2 flex flex-row items-center gap-3">
              <Tooltip
                title={`Calificación ${Math.ceil(
                  realtor?.averageRating || 0
                )} ${getRating(Math.ceil(realtor?.averageRating || 0))}`}
                placement="top-start"
              >
                <Rating
                  readOnly
                  style={{ width: '35%' }}
                  value={Math.ceil(realtor?.averageRating || 0)}
                  itemStyles={ratingStyles}
                  className="cursor-pointer"
                />{' '}
                <p className="text-gray-500 ml-1 font-thin dark:text-white">{`(${Math.ceil(
                  realtor?.averageRating || 0
                )})`}</p>
              </Tooltip>
            </div>
