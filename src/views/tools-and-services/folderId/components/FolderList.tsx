...
const FolderList = () => {
  ...
  const renderedChildrenFolders =
    data?.childrensFolders?.length > 0 ? (
      data?.childrensFolders?.map((folder) => (
        <FolderItem
          key={folder.id}
          folder={folder}
  ...
  return (
    <>
      <div className="flex items-center">
        <h2 className="text-sm font-normal">Carpetas</h2>
      ...
      <ul className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 my-4">
        {isSuccess && renderedChildrenFolders}
      </ul>
    </>
  )
}
