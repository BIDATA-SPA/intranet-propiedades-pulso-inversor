      ...
      >
        <div className="overflow-hidden relative text-center 2xl:p-2 group items-center flex duration-500 h-full ">
          <div className="flex items-center gap-4 bg-transparent group-hover:blur-[1px]  z-10 transition-all duration-500">
            <Avatar
              className={`${avatarClass} md:text-lg md:p-1 2xl:text-xl`}
              // size={avatarSizeXl}
              icon={icon}
            />
...
const PropertyStatistic = () => {
  ...
  const { propertyData } = dashboardData
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-2 mb-3">
      <StatisticCard
        ...
        nameTitle={propertyData?.totalProperties?.nameTitle}
        value={data?.personalInfo?.totalProperties}
        path={propertyData?.totalProperties?.path}
      ...
      <StatisticCard
        ...
        nameTitle={propertyData?.newProperties?.nameTitle}
        value={data?.personalInfo?.newProperties}
        path={propertyData?.newProperties?.path}
        growthRate={propertyData?.newProperties?.growShrink}
        loading={loading}
      />
    </div>
  )
...
export default PropertyStatistic
