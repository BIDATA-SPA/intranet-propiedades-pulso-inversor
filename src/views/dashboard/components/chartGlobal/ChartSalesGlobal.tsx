...
const BasicLine = () => {
  ...
  useEffect(() => {
    if (data?.personalInfo?.metadataKanjesByMonth) {
      ...
      setMonths([...new Set(uniqueMonths)]) // Elimina duplicados
    } else {
...
//     console.log("dahbaord")

//     const dataByMonth = data?.globalInfo?.metadataKanjesByMonth

//     if (dataByMonth && dataByMonth.length >= 0) {
//       // Ordenar los meses izquierda a derecha
//       const sortedDataByMonth = dataByMonth.slice().sort((a, b) => {
...
//         newDataExchange[0].data.push(item.kanjesCompleted)
//         newDataExchange[1].data.push(item.kanjesActives)
//         newDataExchange[2].data.push(item.newKanjes)

//         if (!chartData[0]?.data.includes(formattedDate)) {
//           chartData[0]?.data.push(formattedDate)
//         }
//       })

//       // acorta los meses para ser agregados al eje x de chart
//       const sortMonths = sortedDataByMonth.map((item) => {
//         const date = new Date(item.date)
//         return date.toLocaleString('default', { month: 'short' })
//       })

//       setMonths([...new Set(sortMonths)])
//     } else {
//       console.log('')
//     }
...
// export default BasicLineGlobal
