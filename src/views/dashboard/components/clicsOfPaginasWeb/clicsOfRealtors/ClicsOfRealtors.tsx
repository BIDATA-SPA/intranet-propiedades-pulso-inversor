...
const ClicsOfRealtors = () => {
    ...
    const hasClicks = userClick ? userClick.clickOfMoreOfRealtor +
    ...
    return(
        ...
        <article className="shadow-md group group-hover:blur-lg h-96 w-full sm:h-full border border-1 rounded-md overflow-hidden overflow-y-auto">
            <div className='mx-3 m-2'>
                ...
                <small>realizados en portal corredores en página web oficial <a className='underline underline-offset-1 italic' href='https://procanje.com/portal-corredores' target='_blank' rel='noreferrer'>PulsoInversor</a></small>
            </div>
            { hasClicks ? (
            <>
                <Chart
                options={{
                    colors: COLORS,
                    labels: ['Visita a perfil', 'Resumen de perfil', 'Intentado contactar', 'Contactado por correo ' ,'Han visto tu sitio web'],
                    responsive: [
                        {
                            breakpoint: 480,
                            options: {
            ...
            <div className='flex justify-end mx-4 m-1 mb-2 cursor-pointer'>
                ...
                className='italic hover:scale-x-105 duration-150 text-lime-400'>Ver más detalles</p>
            </div>
            </>    
