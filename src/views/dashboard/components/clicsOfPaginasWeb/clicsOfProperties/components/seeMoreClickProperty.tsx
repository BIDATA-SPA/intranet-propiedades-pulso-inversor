import { Dialog, Progress, Dropdown } from "@/components/ui";
...
const StadisticsClicks = ({dataClicks}) => {
        ...
        switch (sortByData) {
            ...
            case "mostInteractions":
                return [...data].sort((a, b) => {
                    const totalA = (a?.propertyClick?.clickOfExternalLink || 0) 
                                 + (a?.propertyClick?.clickOfOpenContact || 0) 
                                 + (a?.propertyClick?.clickOfSendContact || 0);
                    const totalB = (b?.propertyClick?.clickOfExternalLink || 0) 
    ...
    return(
        <div className="overflow-x-hidden overflow-y-auto p-2 h-full xl:h-[60vh]">
            <div className="flex justify-start items-center mx-2 text-sky-500">
            <Dropdown 
            title="Ordenar por" 
            trigger="hover" 
            className="" 
            ...
            >
                ...
                <Dropdown.Item eventKey="mostInteractions">Más interacciones</Dropdown.Item>
                <Dropdown.Item eventKey="leastInteractions">Menos interacciones</Dropdown.Item>
                {/* <Dropdown.Item eventKey="d">Item D</Dropdown.Item> */}
            </Dropdown>
            </div>
            <div className="flex flex-col gap-3 pt-3 mx-2">
                {
                    sortedData?.length > 0 ? sortedData?.slice(0,3).map(propClick => {
                        return(
                            <article key={propClick?.id} className="flex flex-col overflow-hidden w-full px-1 h-full 2xl:h-full rounded-md border border-sky-50 shadow-md hover:scale-105 duration-200">
                                <div className="flex flex-col sm:flex-row  items-center px-4 sm:px-1">
                                    <div className="w-1.5/3 items-center p-1">
                                        {propClick?.images.length > 0 ? propClick.images.slice(0,1).map(img => (
                                            <div key={img.id}>
                                                <img src={img.path} loading="eager" className="object-cover h-20 w-20 sm:h-24 sm:w-24 rounded-md" alt="" />
                                            </div>
                                        )): 
                                        <img src={'/img/not-found/not-found-image.png'} className=" h-20 w-20 sm:h-24 sm:w-24 rounded-md" alt="" />
                                        }
                                    </div>
                                    <div className="flex flex-col gap-2 mt-1 sm:w-96 sm:mx-6 px-1 sm:pl-2">
                                        <div>
                                            <h6 className="text-sm text-gray-800 font-semibold mt-2 mb-2">{truncate(propClick?.propertyTitle, 36|| 'Sin titulo')}</h6>
                                            <ul className="flex flex-row gap-2 justify-between text-xs mt-1">
                                                <li className="flex flex-col items-center">
                                                    <span className="font-semibold">
                                                    {propClick?.propertyClick?.clickOfExternalLink || 0}
                                                    </span>
                                                    <small className="text-sm">veces</small>
                                                    <small className="font-normal mb-2 text-center">Han revisado link de portal</small>
                                                </li>
                                                <li className="flex flex-col items-center">
                                                    <span className="font-semibold">
                                                    {propClick?.propertyClick?.clickOfOpenContact || 0}
                                                    </span>
                                                    <small className="text-sm">veces</small>
                                                    <small className="font-normal mb-2  text-center">Han revisado para contactarte</small>
                                                </li>
                                                <li className="flex flex-col items-center">
                                                    <span className="font-semibold">
                                                    {propClick?.propertyClick?.clickOfSendContact || 0}
                                                    </span>
                                                    <small className="text-sm">veces</small>
                                                    <small className="font-normal mb-2  text-center">Te han contactado</small>
                                                </li>
                                            </ul>
                                        </div>
                ...
                {
                    !moreProp ? '' : moreProp && (
                        sortedData?.length > 0 ? sortedData?.slice(4, limitTop).map(propClick => {
                            return (
                                <article key={propClick?.id} className="flex flex-col overflow-hidden w-full px-1 h-full 2xl:h-full rounded-md border border-sky-50 shadow-md hover:scale-105 duration-200">
                                <div className="flex flex-col sm:flex-row  items-center px-4 sm:px-1">
                                    <div className="w-1.5/3 items-center p-1">
                                        {propClick?.images.length > 0 ? propClick.images.slice(0,1).map(img => (
                                            <div key={img.id}>
                                                <img src={img.path} className="object-cover h-20 w-20 sm:h-24 sm:w-24 rounded-md" alt="" />
                                            </div>
                                        )): 
                                        <img src={'/img/not-found/not-found-image.png'} className=" h-20 w-20 sm:h-24 sm:w-24 rounded-md" alt="" />
                                        }
    ...
    return(
        <>
            <div className="text-center mb-4 sm:mb-2 mt-4 sm:mt-2 sm:mx-14">
                ...
                <small>
                    en la web oficial
                {' '}<a className='text-sky-600 underline underline-offset-1 italic' href='https://procanje.com/portal-propiedades' target='_blank' rel='noreferrer'>
                    Procanje.com
                </a>
                </small>
            </div>
...
const SeeMoreClicksPropertyDialog =({openDialog, closeDialogs, dataClicks}) =>{
    return(
