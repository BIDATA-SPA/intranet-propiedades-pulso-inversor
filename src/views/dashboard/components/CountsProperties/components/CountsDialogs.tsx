  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 overflow-hidden my-2 mt-4 sm:mx-5 p-4 sm:p-2">
      {percentages.map((item, index) => (
        <article
          key={index}
          className="flex flex-col group items-center p-2 py-3 w-full h-44 sm:h-40 rounded-md border border-sky-500 drop-shadow-2xl hover:scale-105 duration-200"
        >
          <div className="mb-3">
...
const MainContentEvent = ({ closeDialogs, dataRequests }: MainContentEventProps) => {
  return (
    <>
...
const CountsDialogs = ({ openDialog, closeDialogs, dataRequests }: CountsDialogsProps) => {
  return (
